...
public class UploadController {
	...
	public String formUploadFile(@RequestPart MultipartFile[] images) {
		...
		log.info(image.getContentType());  // 개별파일이 데한 종류 구분
		log.info(image.getOriginalFilename()); // 사용자가 넘겨준 실제파일
//		log.info(image.getName());	
	...
	private String makeFolder() {
		...
		// LocalDate를 문자열로 포멧
		String folderPath = str.replace("/", File.separator); // "/" => 자바가 인식못하기때문에 변환작업
		File uploadPathFoler = new File(uploadPath, folderPath); // File 클래스 생성, 밑의 두작업때문에
